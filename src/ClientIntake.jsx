...
After the assessment, if a lawyer is recommended, tell the user you can connect them with one. If you know which lawyer from the AVAILABLE LAWYERS list matches, name them specifically.
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const iv = setInterval(async () => {
      ...
      if (JSON.stringify(l) !== JSON.stringify(lawyers)) setLawyers(l);
    }, 5000);
  ...
  const callAI = async (history, retries = 3) => {
    ...
    if (activeLawyers.length > 0) {
      const lawyerList = activeLawyers.map(l => {
        ...
        const loc = l.city ? `${l.city}, ${l.province}` : l.province;
...
CRITICAL: You DO have access to a verified lawyer directory — the list above. These are real lawyers in our referral network. When recommending legal help:
1. ALWAYS recommend a specific lawyer by name from this list if one matches the user's legal area and province.
2. Say: "I'd recommend speaking with [Name]${activeLawyers[0]?.firm ? ' at [Firm]' : ''} — they handle [area] cases in [province]. Would you like me to connect you?"
3. NEVER say you don't have a directory, can't recommend specific lawyers, or suggest googling. You have the list above — use it.
4. If no lawyer matches the exact area+province, recommend the closest match from the list and note the difference.
5. If the user asks "who is the lawyer" or similar, tell them the matched lawyer's name and firm from this list.
...
NOTE: No lawyers are currently in the referral network. If a user needs a lawyer, recommend they contact their local bar association's referral service or legal aid, and suggest they check back as the network is being built.`;
    }
  ...
  const getMatchingLawyers = () => {
    ...
    return lawyers
      ...
      .filter(l => l.province === assessment.province)
  ...
  const getBroaderMatches = () => {
    ...
    return lawyers
      ...
      .filter(l => l.areas.includes(assessment.legalArea))
      .sort((a, b) => b.rating - a.rating)
      .slice(0, 3);
  };
