...
const BC = { Critical: K.red, High: K.warn, Medium: K.blue, Low: K.textDim, Strong: K.accent, Moderate: K.warn, Weak: K.red, Unclear: K.purple, Active: K.accent, Inactive: K.red, Paused: K.warn, Pending: K.warn, Referred: K.blue, Converted: K.accent, Declined: K.red, Yes: K.accent, No: K.textDim, "Limited-Scope": K.blue, Premium: K.accent, Good: K.blue, Contingency: K.warn, "Legal Aid": K.purple, "Self-Help": K.textDim };
...
function LawyersPage({ lawyers, setLawyers }) {
  ...
  const blank = () => ({ id: uid(), name: "", firm: "", email: "", phone: "", city: "", province: "Nova Scotia", areas: [], languages: ["English"], maxCases: 10, feePerLead: "", feeStructure: "Per Lead", status: "Active", rating: 5, notes: "", created: now() });
    ...
    <div className="fade" style={S.page}>
      ...
      <div style={S.card}>
        <div style={S.grid2}>
          ...
          <Field label="Fee per Lead ($)"><input style={S.input} type="number" value={form.feePerLead} onChange={e => setForm(f => ({ ...f, feePerLead: e.target.value }))} placeholder="50" /></Field>
          <Field label="Fee Structure">
            <select style={S.select} value={form.feeStructure} onChange={e => setForm(f => ({ ...f, feeStructure: e.target.value }))}>
              <option>Per Lead</option><option>Per Conversion</option><option>Monthly Flat</option><option>Revenue Share</option>
            </select>
          </Field>
        ...
        <div style={S.cardFlat}>
          <table style={S.table}>
            <thead><tr>
              ...
              <th style={S.th}>$/Lead</th><th style={S.th}>Structure</th><th style={S.th}>Status</th><th style={S.th}>Rating</th><th style={S.th}></th>
            </tr></thead>
            <tbody>{lawyers.map(l => (
              <tr key={l.id} style={S.tr} onClick={() => startEdit(l)} onMouseOver={e => e.currentTarget.style.background = K.bgHover} onMouseOut={e => e.currentTarget.style.background = "transparent"}>
                ...
                <td style={{ ...S.td, fontFamily: "'IBM Plex Mono', monospace", color: K.accent }}>${l.feePerLead || "—"}</td>
                <td style={S.td}>{l.feeStructure}</td>
...
function CasesPage({ cases, lawyers, setCases }) {
  ...
  const updateStatus = async (caseId, status) => {
    const u = cases.map(c => c.id === caseId ? { ...c, status, converted: status === "Converted" } : c);
    setCases(u); await save("cases", u);
  };

  const filtered = filter === "All" ? cases : cases.filter(c => c.status === filter);
  const statusCounts = { All: cases.length, Pending: 0, Referred: 0, Converted: 0, Declined: 0, Active: 0 };
              ...
              {c.referredLawyer ? (
                <div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 12 }}>
                    <button style={S.btnP} onClick={() => updateStatus(c.id, "Converted")}>✓ Converted</button>
                    <button style={S.btnD} onClick={() => updateStatus(c.id, "Declined")}>✕ Declined</button>
                    <button style={S.btnS} onClick={() => updateStatus(c.id, "Pending")}>Reset</button>
                  </div>
                </div>
              ) : (
                <div>
                          ...
                          {lawyers.filter(l => l.status === "Active").map(l => {
                            ...
                            return (
                              <button key={l.id} style={{ ...S.btnS, textAlign: "left", padding: "8px 10px", border: `1px solid ${areaMatch && provMatch ? K.accent : K.border}`, background: areaMatch && provMatch ? K.accentGlow : "transparent" }}
                                ...
                                <div style={{ fontSize: 10, color: K.textDim }}>{l.areas.slice(0, 2).join(", ")} · ${l.feePerLead}/lead{areaMatch ? " ✓area" : ""}{provMatch ? " ✓prov" : ""}</div>
                              </button>
                            );
                          })}
                        </div>
...
function DashboardPage({ cases, lawyers }) {
  ...
  const convRate = referred > 0 ? ((converted / referred) * 100).toFixed(0) : 0;
  const revenue = cases.filter(c => c.referredLawyer).reduce((s, c) => { const l = lawyers.find(x => x.id === c.referredLawyer?.id); return s + (parseFloat(l?.feePerLead) || 0); }, 0);
  const potentialRev = cases.filter(c => c.status === "Pending" && c.assessment?.lawyerRecommended !== "No").reduce((s, c) => {
    const match = lawyers.filter(l => l.status === "Active" && l.areas.includes(c.assessment?.legalArea));
    return s + (match.length > 0 ? parseFloat(match[0].feePerLead) || 0 : 0);
  }, 0);
  ...
  cases.filter(c => c.referredLawyer).forEach(c => {
    ...
    if (!byLawyer[n]) byLawyer[n] = { referred: 0, converted: 0, revenue: 0 };
    byLawyer[n].referred++;
    if (c.converted) byLawyer[n].converted++;
    const l = lawyers.find(x => x.id === c.referredLawyer.id);
    byLawyer[n].revenue += parseFloat(l?.feePerLead) || 0;
  });
  ...
  return (
    <div className="fade" style={S.page}>
      ...
      <div style={S.statsRow}>
        ...
        <div style={S.stat}><div style={{ ...S.statVal, color: K.accent }}>{converted}</div><div style={S.statLabel}>Converted</div></div>
        <div style={S.stat}><div style={{ ...S.statVal, color: K.accent }}>{convRate}%</div><div style={S.statLabel}>Conv. Rate</div></div>
        <div style={S.stat}><div style={{ ...S.statVal, color: K.accent }}>${revenue}</div><div style={S.statLabel}>Revenue</div></div>
        <div style={S.stat}><div style={{ ...S.statVal, color: K.warn }}>${potentialRev}</div><div style={S.statLabel}>Pipeline</div></div>
      </div>
            ...
            Object.entries(byLawyer).map(([name, d]) => (
              <div key={name} style={{ padding: "10px 0", borderBottom: `1px solid ${K.border}` }}>
                ...
                <div style={{ display: "flex", gap: 16, marginTop: 4, fontSize: 11, fontFamily: "'IBM Plex Mono', monospace" }}>
                  ...
                  <span style={{ color: K.accent }}>CONV {d.converted}</span>
                  <span style={{ color: K.accent }}>${d.revenue}</span>
                  <span style={{ color: d.referred > 0 ? K.textBright : K.textDim }}>{d.referred > 0 ? ((d.converted / d.referred) * 100).toFixed(0) : 0}%</span>
                </div>
              </div>
            ))
          }
      ...
      <div style={S.card}>
        ...
        <div style={S.grid3}>
          ...
          <div><div style={S.label}>Avg Fee / Lead</div><div style={{ ...S.statVal, fontSize: 20, color: K.accent }}>${lawyers.length > 0 ? (lawyers.reduce((s, l) => s + (parseFloat(l.feePerLead) || 0), 0) / lawyers.length).toFixed(0) : 0}</div></div>
        </div>
      </div>
    </div>
  );
